...
- Current SuperNPUBench Tier-0/Tier-1 direct-boot green cases are `MatMul`,
  `TAdd`, `TAbs`, `TSub`, `TSubs`, `TAdds`, `TMul`, `TMuls`, `TMax`, `TMaxs`,
  `TAnd`, and `TOr`. `TAbs`, `TSub`, `TSubs`, `TAdds`, `TMul`, `TMuls`,
  `TMax`, `TMaxs`, `TAnd`, and `TOr` are the first Tier-1 scalar
  arithmetic/logical/unary promotions: each uses a `jcore/<op>.hpp` Linx
  scalar path and a bounded int64 direct-boot source branch, then must pass
